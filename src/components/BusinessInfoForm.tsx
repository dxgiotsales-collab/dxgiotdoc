  ...
  return (
    <div className="flex gap-6 max-w-full">
      ...
      <div className="flex-1 min-w-0 rounded-lg border border-border bg-card shadow-sm p-6 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-x-5 gap-y-3">
          <Field label="최근 자가측정일" placeholder="2026-01-15" value={biz.lastMeasureDate} onChange={(v) => set("lastMeasureDate", v)} />
          ...
          <FileInput label="사업장 위치도" fileName={locationFile?.name || biz.locationFile} onFileSelect={setLocationFile} />

          <div />
          <FileInput label="설치 배치도" fileName={layoutFile?.name || biz.layoutFile} onFileSelect={setLayoutFile} />
        </div>
      </div>
    </div>
  );
